...
// js/sheet.ts
function clampSnapHeight(start, delta, min, max, isTop) {
  const height = start + (isTop ? delta : -delta);
  return Math.min(Math.max(height, min), max);
}
function resolveSnapPoint(value, viewportHeight, rootFontSize) {
...
function openSheet(dialog) {
  ...
  if (snaps && side !== "left" && side !== "right") {
    ...
    const points = snaps.split(/[\s,]+/).map((value) => resolveSnapPoint(value, document.documentElement.clientHeight, rootFontSize)).filter((point) => point > 0);
    if (points.length > 0) {
...
function initSnapPoints(dialog) {
  ...
  const rootFontSize = parseFloat(getComputedStyle(document.documentElement).fontSize) || 16;
  const points = snapsAttr.split(/[\s,]+/).map((value) => resolveSnapPoint(value, document.documentElement.clientHeight, rootFontSize)).filter((point) => point > 0);
  if (points.length === 0) return;
  const min = Math.min(...points);
  const max = Math.max(...points);
  const isTop = side === "top";
  const heightOf = () => dialog.offsetHeight || parseFloat(dialog.style.height || "0") || min;
  const apply = (height) => {
  ...
  const nearest = (height) => points.reduce((best, point) => Math.abs(point - height) < Math.abs(best - height) ? point : best);
  let dragging = false;
  let startY = 0;
  let startH = 0;
  ...
  handle.addEventListener("keydown", (event) => {
    if (event.key === "PageUp") apply(max);
      ...
      const next = event.key === "ArrowUp" ? sorted[Math.min(index + 1, sorted.length - 1)] : sorted[Math.max(index - 1, 0)];
      apply(next);
    } else {
      return;
    }
...
function initSheets(root = document) {
  root.querySelectorAll("[data-sheet-open]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      openSheet(id ? document.getElementById(id) : null);
    });
  });
  root.querySelectorAll("[data-sheet-close]").forEach((btn) => {
    btn.addEventListener("click", () => closeDialog(btn.closest("dialog")));
  });
  root.querySelectorAll("dialog.sheet[data-sheet-snaps]").forEach(initSnapPoints);
}
